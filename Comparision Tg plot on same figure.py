...
#Post Thesis Line:
Program_Running_For=['PMMA Self_Grassia 02kilo_POST_THESIS','PS Self_Grassia 02kilo_POST_THESIS','PVAc Roland 189kilo','PVME Casalini 60kilo','PC Self_Sato 02kilo_POST_THESIS']
Reference_of_Tg_Data=['Grassia','Grassia','Roland','Casalini','Zoller']	#It is just to name final plots. This line plays NO role in code. 
...
for k in range(0,len(Program_Running_For)):
	...
	if Post_Thesis:
		condoTheory_List= 			['PMMA Self_Grassia',		'PS Self_Grassia',		'PVAc Roland',	'PVME Casalini',	'PC Self_Sato']
		condoTheory_Rratio_List=	[2.0,						2.0,					3.0,			3.0,				2.0]
		condoTheory_epsilon_2_List= [4919.899627375042,			5028.423015798441,		6029.978362,	4480.294232,		6109.985544253341]
		...
		myTheory_List=			['PMMA Self_Grassia 02kilo_POST_THESIS',	'PS Self_Grassia 02kilo_POST_THESIS',		'PVAc Roland 189kilo',		'PVME Casalini 60kilo',			'PC Self_Sato 02kilo_POST_THESIS']
		myTheory_Rratio_List=	[1.0029675775730258,						1.0256557135378013, 						1.7905361032016889,		 	1.9119536145328826,		 		0.9160630513838226]
		myTheory_epsilon_2_List=[7054.874048684184,	 						7086.5616869828045, 						6744.040433939719, 			5425.927638003223,		 		8344.843345144749]
		myTheory_x_List=		[0.30252525252525253,						0.3094949494949495,	 						0.3188787878787879,	 		0.29024242424242425,		 		0.3118181818181818]
	...
	plt.plot(P,Tg_calculated_myTheory,color='b',lw=linewidth,ls='-',label='Present Theory')
	plt.plot(P,Tg_calculated_condoTheory,color='r',lw=linewidth,ls='--',label='Condo et al.')
	# plt.plot(P_line,Tg_line,'k',color='r',lw=linewidth,ls='-',label='Ideal Straight Line')
	plt.plot(Pg_exp,Tg_exp,'sk',color='g',ms=markersize,label='Experiment')
	...
	plt.title(' {}) {} - {}'.format(chr(k+97),Polymer_Type,Ref_Tg), fontsize=title_size,x=0.0,y=0.84,horizontalalignment='left')
...
fig.savefig('./'+output_folder+r'\pure_Tg vs P_POST_THESIS_new'+img_extension,dpi=240)
plt.show()
